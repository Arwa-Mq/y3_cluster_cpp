...
// built from a real test-sampler dump
// (/pscratch/sd/j/jesteves/y3cpp_gpu_validate/prj_frozen_gpu_output,
// produced 2026-08-12 by prj_frozen_gpu.ini -- the same run the module's
// own README.md quotes: max |GPU/production - 1| = 1.2e-14 (rnd), 2.2e-11
// (cl), 1.5e-11 (vals) on this 180-point wall). Reference values are the
// module's own previously-recorded output in that dump
// (dsigma_prj_frozen_gpu/{vals,rnd,cl}), i.e. this is a reproducibility
// regression check on the shipped artifact, not a fresh derivation.
...
namespace {
  ...
  dump_dir()
  {
    return "/pscratch/sd/j/jesteves/y3cpp_gpu_validate/prj_frozen_gpu_output";
  }
...
TEST_CASE("ShearPrjFrozenGpu.so reproduces its own recorded GPU output")
{
  ...
  // mirroring [ShearPrjFrozenGpu] in prj_frozen_gpu.ini exactly. ----
  ...
  // Reference: the module's own previously-recorded output on this exact
  // wall (README.md's 2026-08-12 validation run).
  auto const ref_vals = read_column(D + "/dsigma_prj_frozen_gpu/vals.txt");
  auto const ref_rnd = read_column(D + "/dsigma_prj_frozen_gpu/rnd.txt");
  auto const ref_cl = read_column(D + "/dsigma_prj_frozen_gpu/cl.txt");
