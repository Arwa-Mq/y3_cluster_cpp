// Option C (frozen-physics DeltaSigma_prj benchmark backend) -- see
...
// b_sel/DeltaSigma_prj recipe into the cosmosis pipeline" (Part II/Part III,
// Option C) for the design rationale.
//
// Same frozen-physics algebraic reduction as Option E (ShearPrjFrozenCuhre):
// the z-dependent work (exact rnd-channel hoist, cl-channel r_s(M)-anchored
// amplitude drift a_b(z)) is done once per (lob, zob) slice on a fixed
// ring+outer z-grid (build_z_grid_, copied verbatim from sp_detail::
// ShearPrjCore). What differs from Option E: the (theta, lnM) assembly is
// NOT a continuous Cuhre integral over Interp1D-tabulated functions -- it is
// an explicit N_theta x N_M grid + dot product, using sp_detail::
// build_theta_grid (the SAME log-GL, per-R-breakpoint theta grid production's
// own ShearPrjEvaluator uses) and the fixed lnM GL grid (lnm_x_/lnm_w_).
// DSigma_mis(R | theta, M) is cached once per (slice, R) in set_sample(),
// exactly mirroring sp_detail::ShearPrjCore's own Smis_k/DSmis_k caching, so
// evaluate() is a pure dot product with no NFW lookup inside it.
//
// Ω(z) convention: includes the Ω(z) survey-weight factor in common_z,
// matching ShearPrjGsl's convention (confirmed correct empirically against
// Option E: with Ω(z) on, this class validates against ShearPrjGsl, not
// ShearPrjEvaluator/baseline -- see the plan's Part III validation notes for
// why ShearPrjEvaluator's own comment says Ω(z) should NOT appear for a
// surface-density observable, and this module's deliberate choice to differ).
#ifndef Y3_CLUSTER_CPP_SIGMA_PRJ_FROZEN_T_HH
#define Y3_CLUSTER_CPP_SIGMA_PRJ_FROZEN_T_HH

#include "models/sigma_prj_t.hh"   // sp_detail::{PI, R_lambda, default_lob_centers, theta_excl_at_z, build_theta_grid}
